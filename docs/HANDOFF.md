...
>
> Cập nhật: **07/08/2026** — sau đợt làm T1–T10, W3, W5 public web và batch T11a dựng ngưỡng sức chứa theo giờ (schema production đã áp dụng; app đang ở cổng deploy của batch này).
>
...
- **474 bài test cục bộ pass, 1 bài được đánh dấu skip có chủ đích**, `lint`/`typecheck`/`build` sạch ở cổng T11a; số lượt Playwright production sẽ cập nhật sau smoke của batch này.
...
| 5 | **T11** | 🟡 **T11a đã dựng và áp schema; app đang chờ deploy · T11b SOP Go/No-Go kế tiếp** | T11a dùng mô hình vật lý *(số phương tiện × chỗ/phương tiện × 60 ÷ phút/vòng)*, hiện phép tính và nhãn nguồn `ước-lượng` / `khách-cung-cấp` / `đo-thực-tế`; ngưỡng theo giờ tại bến/cổng/bãi xe, T8 chỉ là proxy lượt vào. Migration `037` đã ở production, query thật đã kiểm tra seed/rule/audit/RLS. Chưa đánh dấu T11 xong vì chưa có workflow SOP Go/No-Go và app T11a chưa qua smoke production |
