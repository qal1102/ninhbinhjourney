...
- Production alias đang phục vụ: https://ninhbinhjourney.vercel.app (vẫn đang chạy source đã deploy ngày 29/07; **source AP/NCC dưới đây đã commit và đã push lên `qal1102/ninhbinhjourney/main` (`bd105e4`), xác nhận lại trong phiên tiếp theo bằng clone sạch — vẫn chưa deploy production**).
...
- Chưa xác minh: chưa deploy production với source này (đã commit và đã push lên `main`, chưa deploy); mobile-chromium project của bài E2E mới chưa chạy (mới chạy desktop-chromium, vì bài test này không lặp lại được trên cùng seed đã dùng một lần).
...
1. Batch AP–NCC đã commit và đã push lên `main` (`bd105e4`), build/typecheck/lint/test cục bộ đã xác nhận qua trong phiên tiếp theo (clone sạch). Còn lại trước khi đóng hẳn vòng: deploy production với source này rồi smoke, và chạy mobile-chromium cho `erp-supplier-ap-workflow.spec.ts` (hiện chỉ có desktop-chromium, không lặp lại được trên cùng seed đã dùng).
