...
- **Chưa xong:** chưa commit/push/deploy source code (chỉ mới apply migration lên Supabase); chưa smoke thật trên production để xác nhận nhân viên A đăng nhập thấy đúng quyền quản lý B vừa cấp; `incident-workflow-workspace.tsx`, `project-event-workspace.tsx`, QR scan và field-report vẫn còn giả — chưa đụng tới trong phiên này.
