  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-[#d8e0db] bg-white p-5 shadow-sm sm:p-6">
        ...
        {/* Không có ảnh thì nói là không có ảnh.
            Trước đây thẻ báo cáo thiếu bằng chứng vẫn lấy `site.image` — tấm
            ảnh quảng bá của cơ sở — làm nền, ngay dưới ô đếm "Thiếu ảnh hiện
            trường: 3". Người xem thấy ba tấm ảnh đẹp và một con số nói không
            có ảnh nào; một trong hai đang nói dối. Màn hình này bán chữ "bằng
            chứng", nên chỗ trống phải để trống. */}
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{reports.map((report) => <button key={report.id} type="button" onClick={() => setSelected(report)} className="overflow-hidden rounded-2xl border border-[#dce3df] bg-white text-left transition hover:border-[#8ba99c]">{report.imageUrl ? <div className="aspect-[16/9] bg-cover bg-center" style={{ backgroundImage: `linear-gradient(180deg,transparent,rgba(8,24,18,.55)),url("${report.imageUrl}")` }} /> : <div className="grid aspect-[16/9] place-items-center border-b border-dashed border-[#cfd9d3] bg-[#f4f6f4] px-4 text-center text-xs font-bold text-[#7b8881]">Chưa đính kèm ảnh hiện trường</div>}<div className="p-4"><div className="flex items-center justify-between gap-2"><span className="text-xs font-black text-[#547166]">{report.id}</span><span className="text-xs text-[#7b8881]">{new Intl.DateTimeFormat("vi-VN", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Ho_Chi_Minh" }).format(new Date(report.createdAt))}</span></div><p className="mt-2 font-black text-[#2d4138]">{report.area}</p><p className="mt-1 text-xs text-[#6e7b75]">{report.task}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#edf1ee]"><div className="h-full rounded-full bg-[#397a62]" style={{ width: `${report.progress}%` }} /></div></div></button>)}</div>
      </section>

      {selected && typeof document !== "undefined" ? createPortal(<div className="fixed inset-0 z-[1200] grid place-items-end bg-[#071b15]/55 p-0 backdrop-blur-sm sm:place-items-center sm:p-5"><button type="button" aria-label="Đóng báo cáo" onClick={() => setSelected(null)} className="absolute inset-0" /><section role="dialog" aria-modal="true" aria-label={`Báo cáo ${selected.id}`} className="relative z-10 max-h-[92dvh] w-full overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:max-w-3xl sm:rounded-3xl">{selected.imageUrl ? <div className="aspect-[16/8] bg-cover bg-center" style={{ backgroundImage: `url("${selected.imageUrl}")` }} /> : <div className="grid aspect-[16/8] place-items-center border-b border-dashed border-[#cfd9d3] bg-[#f4f6f4] px-6 text-center text-sm font-bold text-[#7b8881]">Báo cáo này chưa đính kèm ảnh hiện trường</div>}<div className="p-5 sm:p-7"><div className="flex justify-between gap-4"><div><p className="text-xs font-black text-[#477565]">{selected.id} · {selected.category}</p><h2 className="mt-2 text-2xl font-black text-[#20342c]">{selected.area}</h2></div><button type="button" aria-label="Đóng" onClick={() => setSelected(null)} className="grid h-11 w-11 place-items-center rounded-full border border-[#d6dfd9] text-xl">×</button></div><dl className="mt-5 grid grid-cols-2 gap-3 text-sm"><div className="rounded-xl bg-[#f3f6f4] p-3"><dt className="text-xs text-[#7b8881]">Nhân viên</dt><dd className="mt-1 font-black">{selected.employeeName}</dd></div><div className="rounded-xl bg-[#f3f6f4] p-3"><dt className="text-xs text-[#7b8881]">Tiến độ</dt><dd className="mt-1 font-black">{selected.progress}% · {selected.status}</dd></div><div className="rounded-xl bg-[#f3f6f4] p-3"><dt className="text-xs text-[#7b8881]">Công việc</dt><dd className="mt-1 font-black">{selected.task}</dd></div><div className="rounded-xl bg-[#f3f6f4] p-3"><dt className="text-xs text-[#7b8881]">Mã hạch toán</dt><dd className="mt-1 font-black">{selected.financeCode}</dd></div></dl><p className="mt-4 rounded-xl border border-[#dce4df] p-4 text-sm leading-6 text-[#5e6f67]">{selected.note}</p></div></section></div>, document.body) : null}
    </div>
  );
}
