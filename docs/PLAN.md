...
| Camera AI | `[~]` Feed vẫn mô phỏng (chưa có gateway camera thật — xem G13.4). Nút "Giao quản lý kiểm tra"/"Tạo phiếu hiện trường"/"Báo quản lý" **đã nối Supabase thật** ngày 01/08/2026 (migration `202608010017_erp_incident_camera_report.sql`, RPC `erp_incident_report_from_camera`, ghi vào `erp_incidents` có sẵn thay vì bảng mới) — trước đó (xác nhận 31/07) chỉ đổi toast cục bộ, quản lý không bao giờ thấy. Đã deploy và smoke thật trên production bằng Playwright hai phiên tách biệt (`prod-smoke-camera-ai-incident.spec.ts`, 2/2 pass) | Gateway thật, quyền, health, event, privacy, xác minh và retention |
...
**Bằng chứng 01/08/2026 (V13):** phần "SLA" của mục đầu tiên ở trên nay đã thật — `elapsed_minutes` (số cứng ghi một lần lúc seed, không bao giờ tính lại) đã bị xoá hẳn; migration `202608010015_erp_incident_sla_clock.sql` + `202608010016_erp_incident_sla_clock_backfill_fix.sql` thêm `reported_at_ts timestamptz` thật, `incident-repository.ts` tính `elapsedMinutes` tại thời điểm đọc (đang mở thì chạy theo `now()`, đã đóng thì đông cứng theo `updated_at`). Xác minh trực tiếp trên production bằng `supabase db query --linked`: sự cố đang mở hiện đúng ~1170 phút đã trôi qua kể từ lúc seed 31/07 (đồng hồ chạy thật, không còn đứng yên). Vẫn chưa làm: `assign`/`triage`/`contain`/`review` thành bước riêng, và **tự động chuyển cấp khi quá hạn** (cần cron/edge function — đây là V15 trong file đánh giá) nên checkbox này giữ nguyên `[ ]`.

**Bằng chứng 02/08/2026 (V15):** mục "Escalation giám đốc chỉ khi severity/ngưỡng/**overdue**" nay có phần *overdue* chạy thật. Migration `202608020024_erp_incident_auto_escalation.sql` bật `pg_cron`, chạy `erp_incident_escalate_overdue()` mỗi phút: sự cố chưa đóng và quá `sla_minutes` sẽ tự đặt cờ `escalated`, ghi lý do có số liệu và thêm một dòng nhật ký ghi rõ **"Hệ thống"** là người thực hiện. Đây là **cơ chế chạy theo thời gian đầu tiên của toàn hệ thống** — trước đó mọi thứ chỉ xảy ra khi có người bấm. Cố ý không đổi `severity` (chuyển cấp ≠ mức nghiêm trọng) và không đụng `next_action` (thuộc máy trạng thái chuyển bước). Xác minh trực tiếp trên production: **không gọi RPC bằng tay**, chờ cron tự chạy rồi đọc `cron.job_run_details` (`succeeded`, 0 lượt lỗi) và thấy 4 sự cố quá hạn đã tự chuyển cấp; chạy lại nhiều lượt không nhân bản dòng nhật ký (giữ nguyên 4 dòng) nhờ bộ lọc `escalated = false`. Playwright `prod-smoke-incident-auto-escalation.spec.ts` **6/6 pass**. **Checkbox vẫn `[ ]`** vì `triage`/`assign`/`contain`/`review` chưa tách thành bước riêng, evidence vẫn read-only, và chưa có RCA/CAPA sau khi đóng hồ sơ.
...
**Bằng chứng 01/08/2026 (V4):** dòng thứ 5 ở trên (“Alert phải có người xác minh”) nay có nửa sau thật — bấm hành động trên một cảnh báo tạo đúng hồ sơ `erp_incidents` thật (RPC `erp_incident_report_from_camera`, migration `202608010017`), quản lý xác minh qua đúng luồng tiếp nhận/giao/đóng có sẵn của module Sự cố. Vẫn chưa có registry/stream/quyền xem/AI detection thật — feed camera vẫn hoàn toàn mô phỏng, checkbox này giữ nguyên `[ ]`.
