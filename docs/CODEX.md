...
- Chưa xác minh: đăng nhập vai trò thật trên production để xem `SupplierApControlCenter` sau deploy (mới smoke HTTP status); mobile-chromium project của bài E2E AP chưa chạy (mới chạy desktop-chromium, vì bài test này không lặp lại được trên cùng seed đã dùng một lần).
...
1. Batch AP–NCC đã commit, đã push (`bd105e4`, `331bb1d`) và **đã deploy production** (`dpl_HfdkxgSwDubYZDE4Kt9YukegdxXK`, xác nhận qua `vercel inspect`). Còn lại trước khi đóng hẳn vòng: đăng nhập vai trò thật trên production để smoke sâu (không chỉ HTTP 200) cho `SupplierApControlCenter` và các màn cũ, và chạy mobile-chromium cho `erp-supplier-ap-workflow.spec.ts` (hiện chỉ có desktop-chromium, không lặp lại được trên cùng seed đã dùng).
