...
- Local gate mới nhất 21/08: `release:preflight` pass trọn gói với đúng Vercel project; typecheck/lint/build pass; 96 file Vitest pass + 1 skip, 607 test pass + 1 skip. Playwright A6 desktop/mobile 4/4 pass. Commit A6 `48d48b3` đã push/deploy; production smoke read-only với expectation `blocked` pass 1/1 và xác nhận gate trả `CHƯA ĐƯỢC BẬT PRODUCTION`.
...
- Vercel đã cấu hình experience mode production, ba policy version, AES/HMAC key/version và đủ 10 flag; không ghi giá trị secret vào tài liệu. Cấu hình cũ trong `vercel.json` từng ép build về `client-demo` đã bị loại bỏ và có unit contract chống tái diễn.
- Production deployment ứng dụng đầu tiên đạt gate là `dpl_EfHq65vZgcJs89jd5sqYbhX6SVpF`; alias `https://ninhbinhjourney.vercel.app` trả HTTP 200 với `experienceMode=production`. A6 director smoke read-only đạt **2/2** desktop/mobile với expectation `canary-ready`; các deployment docs-only sau đó không đổi app source đã smoke.
- Public smoke đạt **2/2** desktop/mobile cho campaign Trung thu; smoke điều hướng đạt **2/2** từ sản phẩm `Bàn Trăng bên Ngô Đồng` tới checkout có nút giữ chỗ ERP. POST ngày quá hạn tới endpoint production trả `CUSTOMER_BOOKING_INPUT_INVALID` thay vì `CUSTOMER_BOOKING_DISABLED`, xác nhận feature đang hoạt động mà không tạo order/hold thử nghiệm.
- Consent center vẫn hoạt động trên bề mặt khách hàng nhưng không còn render tại `/erp` hoặc `/ops`; hotfix này loại bỏ lớp phủ từng chặn nút đăng nhập ERP khi các flag được bật.
