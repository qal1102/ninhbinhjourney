    ...
    <div className="space-y-6" data-testid="marketing-qr-control-center">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <form action={campaignAction} className="rounded-3xl border border-[#d8e0db] bg-white p-5 shadow-sm">
          ...
          <label className="mt-4 grid gap-1 text-xs font-bold text-[#5d6f66]">Tên nội bộ<input name="name" required minLength={2} maxLength={160} placeholder="QR bến Tam Cốc tháng 8" className="min-h-11 rounded-lg border border-[#cbd7d1] px-3 text-sm" /></label>
          ...
          <p className="mt-2 text-xs text-[#7c8b83]">Mã chiến dịch để máy tự đặt theo tên, dùng in lên biển ngay sau khi tạo.</p>
        ...
        <form action={sourceAction} className="rounded-3xl border border-[#d8e0db] bg-white p-5 shadow-sm">
          ...
          {config.campaigns.length === 0 ? <p className="mt-4 text-sm text-[#66756e]">Cần tạo chiến dịch trước khi tạo mã QR.</p> : <>
            ...
            <p className="mt-2 text-xs text-[#7c8b83]">Mã QR và mã vị trí để máy tự đặt từ chiến dịch và nhãn vị trí, dùng in lên biển ngay sau khi tạo.</p>
