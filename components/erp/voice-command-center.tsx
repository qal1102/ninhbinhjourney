...
const moduleCommands: ModuleCommand[] = [
  ...
  { id: "suc-chua", terms: ["suc chua", "luong khach", "qua tai", "mat do khach", "xe trung chuyen", "xe dien"] },
      ...
      {open && typeof document !== "undefined" ? createPortal(
        <div className="pointer-events-none fixed inset-0 z-[1100]">
          ...
          <section role="dialog" aria-modal="true" aria-labelledby="voice-title" className="pointer-events-auto absolute inset-x-3 bottom-3 max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-3xl border border-[#d5ddd8] bg-[#f5f7f4] p-4 shadow-2xl sm:bottom-5 sm:left-auto sm:right-5 sm:w-[25rem] sm:p-5">
            ...
            >
              ...
              {thread.map((entry) =>
                entry.kind === "reply" ? (
                  ...
                  >
                    {entry.kind === "voice" ? (
                      <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-[#6f8d7f]">
                      ...
                      <p className="text-xs font-black uppercase tracking-[0.12em] text-[#8b9a92]">
                        Lệnh nhập
                      </p>
                    )}
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-black uppercase tracking-[0.14em] text-[#718078]">Lệnh nhanh</p>
                {thread.length ? (
                  <button type="button" onClick={() => setThread([])} className="text-xs font-bold text-[#7d8c84] underline underline-offset-2">
                    Xoá hội thoại
                  </button>
                ) : null}
              </div>
