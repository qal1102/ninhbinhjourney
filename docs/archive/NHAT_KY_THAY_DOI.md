...
- **Kết quả nằm ở `docs/RA_SOAT_GOC_NHIN_KHACH_HANG.md`.** Mọi khẳng định trong đó đều kiểm trực tiếp trên mã nguồn hoặc production ngày 02/08/2026, không chép lại tài liệu cũ.
- Ba phát hiện nặng nhất, đều kiểm ở tầng cơ sở dữ liệu: (a) `erp_gate_scan_events` **không có khoá ngoại tới vé/booking, không ràng buộc `unique`, không idempotency key** — cổng soát vé nhận bất kỳ chuỗi 6 ký tự nào và ghi trùng được; (b) chốt ca dừng ở `posted`, **không có bước nộp quỹ/ngân hàng**; (c) công nợ NCC dừng ở `posted`/`reversed`, **không có trạng thái `paid`**. Hệ thống kiểm soát chặt khúc giữa nhưng hở cả hai đầu dòng tiền.
- Ba thứ mất điểm nặng với người mua nhưng **rẻ nhất để sửa**: ba hệ điều hành song song cùng chạy công khai trên production (`/erp` cookie phiên, `/ops` Supabase Auth, `/demo/ops` số liệu hư cấu — cả ba đều HTTP 200); 5/15 module vẫn là bảng số tĩnh (trong đó có Sức chứa và SOP, đúng hai thứ khách nhấn mạnh nhất); mật khẩu 9 tài khoản in thẳng trên `/erp/login` production.
- Web công khai: thanh toán **bị chặn ở tầng cấu hình** (`config/experience.ts` cấm production bật sandbox checkout, và không có cổng thật) nên nửa thương mại chưa tồn tại; web có 8 điểm còn ERP vận hành 4, **Tam Chúc không xuất hiện lần nào trên web** dù là cơ sở dùng nhiều nhất trong ERP.
- Về account switcher: đã có (V3) nhưng chỉ giám đốc thấy, **chỉ nhảy được một chặng** (phải quay về giám đốc mới đổi tiếp) và nút nằm khuất — nên đúng là chưa giải quyết được việc khách cần. Đề xuất cho nhảy thẳng giữa hai vai trò (an toàn không đổi vì `actingAsFor` vẫn giữ danh tính giám đốc thật), thêm thanh trình diễn luôn hiện, ẩn mật khẩu theo biến môi trường, và **bắt buộc tắt được khi bàn giao**.
