...
>
> Cập nhật: **07/08/2026** — T11b đã áp dụng schema Go/No-Go lên production; app đã qua toàn bộ cổng local và đang chờ commit/deploy/smoke production.
>
...
> **Trạng thái ERP mới nhất 07/08:** migration `202608070038_erp_sop_go_no_go.sql` đã lên Supabase production và khớp local/remote. 20/20 checklist seed đều mang nhãn `demo-unapproved`, đúng 5 mục/cơ sở; 4 quản lý đều có quyền SOP; 4 bảng bật RLS; 2 RPC là `security definer`. Round-trip trong transaction xác minh submit → chặn GO khi lỗi trọng yếu → chấp nhận rủi ro có văn bản, sau đó `ROLLBACK`; query cuối vẫn **0 assessment / 0 result / 0 audit**, không để lại dữ liệu thử. App T11b đã qua 491 test + 1 skip, lint/typecheck/build, nhưng **chưa deploy ở thời điểm ghi dòng này**.
...
- **491 bài test cục bộ pass, 1 bài được đánh dấu skip có chủ đích**, `lint`/`typecheck`/`build` sạch ở cổng T11b; smoke production T11b còn chờ app deploy.
...
| 5 | **T11** | 🟡 **T11a đã deploy + smoke · T11b schema production, app chờ deploy/smoke** | T11a có ngưỡng theo mô hình vật lý và proxy T8. T11b có workflow quản lý gửi → giám đốc quyết định, critical fail chặn GO, risk acceptance bằng văn bản, row lock/version/idempotency/audit. Migration `038` và round-trip `ROLLBACK` đã xác minh production, không để lại dữ liệu thử. Chỉ đánh dấu T11 xong sau khi app T11b deploy và smoke chỉ-đọc xanh |
